...
*[INSERT A FEW BACKGROUND SENTENCES: TMS UTILIZES CERTAIN SEQUENCES THAT ARE THOGUHT TO BE GLU-MODULATING. ALS AND HUNTINGTON'S CYTOTOXICITY. MOTOR= MAJOR GLU OUTPUTMAKE INTELLECTUAL GAP CLEAR. WE DON'T YET KNOW THAT MUCH ABOUT GLU CONCENTRATIONS IN MOTOR VS VISUAL. MOTOR DISTURBANCES IN PSYCHIATRIC DISORDERS.] To investigate this, we're using GluCEST (Glutamate Chemical Exchange Saturation Transfer), a novel invivo glutmate imaging method. We are focusing on behavioral score measuring visual and motor function, and seeing if these score correlate with GluCEST signal. We are also focusing on the healthy population to quanitfy baseline glutmate levels. This is important because we need to understand basic physiology, etc*
...
<br>
<br>
2. Run [/Processing_Pipeline/structural_script.sh](LINK TO SCRIPT ON GITHUB) to process raw 7T Terra MP2RAGE structural data.

    > This script executes the following: UNI and INV2 dicom to nifti conversion, structural brain masking, ANTS N4 bias field correction, FSL FAST (for tissue segmentation and gray matter probability maps), UNI to MNI registration with ANTS SyN (rigid+affine+deformable syn). Note, our major update was to use a 4-compartment FAST segmentation based on UNI file, vs. 3-compartment based on INV-2 file.
<br>
3. Run [/Processing_Pipeline/glucest_script.sh](LINK TO SCRIPT ON GITHUB) to process the 7T Terra GluCEST data output by cest2d_TERRA_SYRP.
...
<br>
4. Run [/Processing_Pipeline/motor_heatmap.sh](LINK TO SCRIPT) to generate a reward network map that only includes voxels for which greater than 50% of participants had GluCEST data (used for coverage sensitivity analysis).

### Measure Extraction
1. Execute [/Measure_Extraction/glumeasureexraction061621.sh](LINK TO SCRIPT) to calculate GluCEST % contrast and gray matter density measures for each subject in all masks of interest.
...
1. Knit [/R_Statistics/Prepare_Demographics_ClinicalMeasures_Data.Rmd](LINK TO SCRIPTS).
